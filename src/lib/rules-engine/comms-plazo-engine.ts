...
/**
 * ITEM-024: "un mes" del art. 176.1 LSC se computa de fecha a fecha
 * (art. 5.1 CC), no como 30 días. Si el día no existe en el mes destino
 * (p.ej. 31/07 → junio), el límite es el último día de ese mes.
 */
export function subtractOneMonthFechaAFecha(d: Date): Date {
  const out = new Date(d);
...
function calcularPlazoConvocatoria(input: PlazoComunicacionInput): PlazoComunicacionResult {
  ...
  if (organo_tipo === 'JUNTA_GENERAL') {
    ...
    // SL = 15 días naturales (art. 176.1 LSC — el 173 regula la FORMA, no el plazo).
