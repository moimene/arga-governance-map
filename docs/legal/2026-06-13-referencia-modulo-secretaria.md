...
| **W10** | Inteligencia jurídica supervisada (redacción asistida de actas desde transcripción, extracción semántica de acuerdos, board pack automático) | — | V3 | Evolutivo | Backlog; **construir sobre evidencia y reglas, no antes** |
...
| **W10** Inteligencia jurídica supervisada (IA) | ⛔ **Bloqueado (infra LLM + consentimiento)** | Requiere endpoint LLM real (proxy server-side + API key del proveedor) **y** consentimiento de *data-egress* (enviar actas/transcripciones a un tercero LLM) — decisión de gobierno del cliente. Construir una "IA falsa" violaría el principio del proyecto (los informes avisan contra IA visible sobre fundamentos no cerrados). Listo para construir la *scaffolding* honesta (edge function + UI gateada *fail-closed*) cuando se provisione el proveedor y se autorice el egress. |
