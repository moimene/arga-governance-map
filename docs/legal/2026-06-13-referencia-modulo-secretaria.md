...
| **W3** Calidad de datos (`data_class`) | ◑ **Parcial** | **Clasificación hecha**: columna `data_class` (CHECK DEMO/TEST/PRE/PROD) en entities+persons; 18 entities + 35 persons de E2E/pruebas marcadas TEST. **Filtrado operativo diferido** a propósito: la revisión `/codex` mostró que filtrar solo un read-path crea inconsistencia (la sociedad TEST seguiría visible en scope/wizards) y no es durable (los builders E2E no setean `data_class`). El filtrado consistente en todos los read-paths + el tagging del harness E2E es un follow-up coordinado. |
...
| **W10** Inteligencia jurídica supervisada (IA) | ◑ **Construido y desplegado; pendiente solo de provisión de key** | **Corrección de diagnóstico (2026-06-14):** el núcleo NO estaba por construir — ya existe `motor-plantillas@1.0.0-beta` (`src/lib/motor-plantillas/`), un copiloto documental gobernado y testeado. `suggestActaDraftPolish()` asiste el **borrador editable** del acta proponiendo reemplazos localizados sobre fragmentos exactos (targets: `narrativa.introduccion`, `narrativa.deliberaciones`, `narrativa.incidencias_no_criticas`), con **harness de protección de hechos jurídicos** (sociedad, órgano, asistentes, quórum, capital, votos, texto de acuerdos, pactos, conflictos, fechas, hashes, snapshots, orden del día) que solo aplica una propuesta si conserva la estructura RRM y pasa `validateRenderedActaAgainstLegalStructure()`. La inferencia vive **server-side** en la edge function `openai-capa3-document-copilot` (**desplegada ACTIVE en Cloud, `verify_jwt:true`**, OpenAI Responses API + Structured Outputs, default `gpt-5.5` temp 0). Ya está **cableada en la UI** (`ProcessDocxButton`, usado en ActaDetalle/GenerarDocumentoStepper/ExpedienteAcuerdo). **Fail-closed por diseño:** sin `OPENAI_API_KEY[_2]` server-side, la función cae al fallback local determinista `capa3-local-demo-assistant`. **Único paso pendiente:** provisionar el secret OpenAI en `governance_OS` (acción del usuario — la sandbox impide leer `.env`; al ejecutarla, el usuario autoriza explícitamente el *data-egress* a OpenAI). Toda sugerencia IA queda **pendiente de revisión humana** (el Secretario revisa y cierra). |

**Gates del run:** `typecheck` ✓ · `build` ✓ · `bun test` 2020 pass / 0 fail (post-W9) · 11 migraciones Cloud aplicadas y espejadas · `main` consolidado y pusheado. Cada fase pasó revisión adversarial `/codex` (W5 requirió 5 rondas hasta veredicto CLEAN; W9, 2 rondas). **W10 no añade código** — fue una corrección de diagnóstico: el núcleo IA (`motor-plantillas`) ya existía construido, testeado, desplegado y cableado; solo restaba provisionar la key.
