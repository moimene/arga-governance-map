...
Hemos convertido tu matriz en el catálogo de obligaciones de AIMS 360. Para hacerlo la hemos leído entera, celda a celda, y la hemos cotejado con el Reglamento en su versión consolidada. Este documento recoge lo que encontramos y lo que necesitamos que nos decidas tú. En el programa **tu criterio manda**. Cuando el texto parece decir otra cosa, aplicamos el texto de forma provisional, lo marcamos «a validar por el experto» y te lo traemos aquí. Ninguna de esas marcas es definitiva hasta que la veamos contigo.
...
- **La presentación de 15 diapositivas.** La citamos como «diap. N».
...
No es cortesía. Son cambios que ya estamos haciendo en AIMS 360 porque tu enfoque es el correcto:

- **Rol por sistema, y múltiple** (Fases C7: «Determinar el rol de la entidad para cada sistema (puede ser varios)»). Nosotros derivábamos un único rol excluyente, así que un sistema desarrollado y usado en casa perdía los arts. 26 y 27.
- **El art. 50 como obligación que se suma a cualquier nivel de riesgo**: «No necesariamente alto riesgo» en OB-42 a OB-46. Respeta el art. 50.6; nosotros lo tratábamos como un nivel.
...
El Reglamento (UE) 2026/1744, el «Ómnibus digital de IA», se publicó en el DOUE el 24-7-2026 y está en vigor desde el 27-7-2026. Suponemos que tu matriz es anterior a esa fecha: no lo recoge, y es lo lógico. Las consecuencias para el catálogo son estas:

- **Art. 4.** Pasa a exigir que se «adopten medidas» y dice expresamente que no exige garantizar un nivel específico. OB-09 se acredita con las medidas adoptadas, no con un resultado.
...
| I-02 | El estado y el % son independientes: nada impide un «Completado» al 50 %. | OB-02: `J3 = Completado`, `K3 = 0,5`, cuando la Leyenda C6 define Completado como 100 %. Además, `K2:K66` solo valida «decimal 0», sin tope, y la columna F (Rol principal) es texto libre, aunque de su literal dependen los `COUNTIF` del bloque por rol (hoy cuadran: suman 65). | El dato es ilustrativo. El problema es que el libro no impide la incoherencia. En la herramienta, el estado y el avance se derivarán uno del otro. |
...
| I-11 | Entregable de OB-48 cortado. | Nuestro volcado del Excel corta siete celdas en 179-180 caracteres: `I11`, `I16`, `I43`, `I49`, `I54`, `I56` e `I65` (OB-10, 15, 42, 48, 53, 55 y 64). OB-48 acaba en «…cumplir sus obligaciones en virtud del pres». En el HTML, d62 trae el texto completo. | **Probablemente es un límite de nuestra extracción, no del Excel.** Como la capa literal del catálogo guarda tu texto íntegro, **P-08.** ¿Nos confirmas el texto completo de esas siete celdas? |
| I-12 | El cuadro digital no admite «no aplica». | Estados del HTML: pendiente, en curso, completado e incidencia (`:107`). El Excel sí lo tenía: Leyenda C4-C5, «N/A excluido del promedio». A un responsable del despliegue puro no le aplican unos 21 entregables: d3-d5 (art. 5.2), d8 y d61-d63 (uso general), d34-d41 (importador y distribuidor), d52 y d69 (autoridades públicas) y d64-d67 (pruebas en condiciones reales). O nunca llega al 100 %, o tiene que marcar «Hecho» lo que no le toca. «Acciones requeridas» propone además «Iniciar» sobre entregables que no aplican (`:185-189`). | En la herramienta, «no aplica» se deriva del rol y los hechos de cada sociedad y sistema, siempre con motivo. |
| I-13 | La fecha de cada entregable no alimenta ninguna alarma. | El campo fecha se guarda y se vuelve a pintar (`:209`) y viaja en la exportación (`:216`), pero `alarms()` solo mira la fecha de la fase (`:178`). | En la herramienta, las alarmas miran la fecha de cada entregable y, además, la fecha de aplicación de cada obligación. |
...
| I-16 | Erratas. | «alfebitización» (`I10`, d12); «Participación en Sanbox» (`C53`); «No necesariamento» (`E46`, `E47`); «en el mercados» (`G2`); «medias adoptadas» por «medidas» (d18); «hardware generados» (d25), donde el art. 13.3 e) dice «necesarios». | Las corregimos en la capa canónica y dejamos intacta tu capa literal. |

---

## 5. Discrepancias de cita o de sujeto

En cada fila, la cita o el sujeto que aparecen en la matriz no casan con el texto. Hemos aplicado el texto de forma provisional y lo hemos marcado. Para cada una necesitamos que la **aceptes** (se queda la cita del texto) o la **rechaces** (se queda tu criterio y Legal documenta por qué).
...
| P-23 | **OB-18 (art. 16) se mantiene como una sola fila.** | OB-18: «Checklist de cumplimiento» (`I19`). El art. 16 tiene letras a) a l). | La mantenemos como **una fila**, con una marca de estado por letra. Así no cambia tu denominador y una obligación con letras pendientes no puede cerrarse. Solo necesitamos tu conformidad. |
...
- **Cláusulas para entidades financieras**, decisivas para una aseguradora: arts. 9.10, 17.4, 18.3, 19.2, 26.5 (párrafos segundo y tercero), 26.6 (párrafo segundo) y 72.4 (párrafo segundo), y el art. 74.6 para la autoridad.
...
- **Correcciones de la verificación que se han respetado:**
  ...
  - I-11 atribuye el corte a nuestra extracción.
