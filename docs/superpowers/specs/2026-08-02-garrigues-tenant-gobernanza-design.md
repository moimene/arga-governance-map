...
- `config.informe_preceptivo_de: [materias]` en el Consejo de Socios → alimenta el gate PRE cuando adopta la Junta (conecta con `InformesPreceptivos.tsx` existente).
...
~120 personas únicas de la web de gobierno corporativo, campos mínimos: nombre + categoría + cargos en comités. Sin datos de contacto ni nada no publicado. Vocabulario de categorías propio de despacho (nuevo respecto a ARGA): `SOCIO, OF COUNSEL, COUNSEL, DIRECTOR GENERAL, DIRECTOR, GERENTE, ASOCIADO PRINCIPAL, ASOCIADO SENIOR, SUPERVISOR, COMPLIANCE OFFICER, SUPPORT, SECRETARY, LEGAL SUPPORT, UTTAI`. Nota RGPD en seed y docs: datos personales de fuente pública del propio titular, finalidad demo, minimización aplicada.
...
### G7 — AI Governance sobre PI-30 (M)
- Inventario `ai_systems` del despacho (ámbito PI-30: IA generativa corporativa, productos g-digital, copilotos internos) — verosímil y etiquetado.
- Comité de Gobernanza de la IA como órgano vinculado (dependencia dual visible); incidentes IA con alta owner-write existente; `evaluaciones/nuevo` sigue bloqueado por RLS `42501` → read-only, o migración RLS pequeña (decisión abierta D-4).
...
- La narrativa circular que cierra la venta: *el grupo gobierna su propia filial de confianza digital con la plataforma, y la plataforma custodia su evidencia en esa misma filial.*
- Runbook de demo (patrón `2026-07-06-demo-runbook.md`) orientado a audiencia CID/Nuevos Negocios Digitales.

## 5. Decisiones abiertas (no bloquean G0)

- **D-1 Login por tenant:** variante `?tenant=garrigues` (recomendada) vs login neutro de plataforma.
- **D-2 Composición del CdA de EAD Trust:** aportar fuente interna o cargar cargos "no verificado".
- **D-3 Materias con informe preceptivo del Consejo de Socios:** fuente interna (estatutos/protocolo) o set demo etiquetado.
- **D-4 Evaluaciones IA:** read-only vs migración RLS para activar el alta.
...
| Datos personales reales | Fuente pública del titular, minimización, nota RGPD, sin contacto |
