-- Codex P2 round 16: mover no-op check después de RBAC/tenant en RPC
-- reclassify_agenda_item_kind. Cierre side-channel info leak cross-tenant.
...
BEGIN
  ...
  FROM agenda_items WHERE id = p_agenda_item_id AND meeting_id = p_meeting_id;
  ...
  IF NOT fn_secretaria_is_service_role() THEN
    ...
    SELECT EXISTS (
      ...
      WHERE ur.user_id = v_user_id AND ur.tenant_id = v_agenda_tenant_id
        AND ur.is_active = true AND r.role_code = 'SECRETARIO'
    ...
    END IF;
  END IF;
  -- No-op check AFTER authorization (Codex P2 round 16)
  SELECT kind INTO v_current_kind
  FROM agenda_items WHERE id = p_agenda_item_id AND meeting_id = p_meeting_id;
  IF v_current_kind IS NOT NULL AND v_current_kind = p_new_kind THEN
    RAISE EXCEPTION 'P7: el punto ya está clasificado como % — reclasificación no-op rechazada.', p_new_kind;
  END IF;
  SELECT m.status, COALESCE((m.quorum_data->>'is_universal')::boolean, false), gb.body_type
  ...
  IF v_meeting_status = 'CANCELADA' THEN
    RAISE EXCEPTION 'P7: reclasificación bloqueada (meeting CANCELADA).';
  END IF;
  IF v_meeting_status IN ('CONVOCADA', 'CELEBRADA') THEN
    ...
    IF v_body_type_upper IN ('JUNTA', 'JUNTA_GENERAL', 'JGA', 'JUNTA_ACCIONISTAS')
      ...
      RAISE EXCEPTION 'P7: junta convocada formalmente — no se puede elevar a DECISORIO (art. 174 LSC).';
    END IF;
  END IF;
