...
2.7 **Huecos de partida — comprobados:** (1) Creación de comisiones post-alta: DESMENTIDO. Existe interfaz en `/secretaria/catalogo-organos` y se materializó la `Comisión de Retribuciones` en 2.1 vía RPC canónica `fn_secretaria_upsert_organ_profile`; (2) Cambio de matriz o porcentaje post-alta: CONFIRMADO que no existe pantalla post-alta (trazado en MOI-148).
...
| H-07 | 2 | `/secretaria/sociedades/nueva` (Sociedad 2.1 Matriz `Corporación Nueva, S.A.`, SA) | Proceso completado por los 11 pasos del asistente. En paso 9 (Reglas), configurado Consejo de Administración con quórum ordinario y voto de calidad de presidente habilitado conforme a estatutos y LSC. Asignados 2 socios (60%/40%), 4 órganos iniciales, 4 cargos (Presidente, Secretaria, 2 Consejeros). Se ejecutó TX1 y TX2 con éxito. La RPC `fn_promover_sociedad_operativa` promovió la sociedad a `OPERATIVA` con `data_class = 'DEMO'`. | Matriz creada y operativa. | C | `2.1-matriz-paso-1-identificacion-relleno.png` a `2.1-matriz-creada-resultado.png`, DB row verificada | Sin hallazgos. Conforme a diseño. |
| H-08 | 2 | `/secretaria/sociedades/nueva` (Sociedad 2.2 Filial A `Servicios Nuevos Integrales, S.L.U.`, SLU) | La matriz 2.1 aparece correctamente como opción seleccionable en el paso Perfil. 100% participación asignada a la matriz en Cap Table. En paso 9 (Reglas), configurado Administrador Único (el voto de calidad es inaplicable al ser órgano unipersonal). Administrador Único designado. TX1 y TX2 ejecutados con éxito (`data_class = 'DEMO'`). El estado final quedó en `INCOMPLETA_CARGOS` porque la RPC exigía Consejo. Resuelto por migración en **MOI-219** y acción interactiva en ficha en **MOI-234**, quedando en estado `OPERATIVA`. | Estado `OPERATIVA`. | A | `2.2-filiala-creada-resultado.png`, DB row verificada | Resuelto en **MOI-219** (soporte de Administrador Único) y **MOI-234** (botón de re-promoción en ficha). |
| H-09 | 2 | `/secretaria/sociedades/nueva` (Sociedad 2.3 Filial B `Tecnología e Innovación Nueva, S.L.`, SL) | La matriz 2.1 aparece como opción en Perfil. Participación 70% matriz + 30% minoritario Alianza Norte. En paso 9 (Reglas), configurada Administración Solidaria (sin voto dirimente al no ser colegiado). Dos Administradores Solidarios designados. TX1 y TX2 ejecutados con éxito (`data_class = 'DEMO'`). El estado inicial quedó en `INCOMPLETA_CARGOS`. Resuelto por migración en **MOI-219** y acción interactiva en **MOI-234**, quedando en estado `OPERATIVA`. | Estado `OPERATIVA`. | A | `2.3-filialb-creada-resultado.png`, DB row verificada | Resuelto en **MOI-219** (soporte de Administradores Solidarios) y **MOI-234** (botón de re-promoción en ficha). |
...
| H-13 | 2 | `/secretaria/catalogo-organos` (Punto 2.7 Hueco 1) | **DESMENTIDO Y EJECUTADO**: Sí existe interfaz para crear órganos o comisiones después del alta en `/secretaria/catalogo-organos`. Creada y materializada en Cloud DB la `Comisión de Retribuciones` (`c9abe18b-6a12-466e-a5e9-87e5ea6adf70`) en la Matriz 2.1 vía RPC gobernada `fn_secretaria_upsert_organ_profile`. | Se sospechaba que no existía pantalla para crear comisiones post-alta. | C | `2.7-crear-organo-modal.png`, `2.7-hueco-1-catalogo-organos.png`, row `c9abe18b` en `governing_bodies` | Desmentido el hueco en documentación: la funcionalidad existe y está probada de extremo a extremo. |
| H-14 | 2 | `/secretaria/sociedades/:id` (Punto 2.7 Hueco 2) | **CONFIRMADO**: No existe interfaz ni botón para modificar la matriz ni el porcentaje de titularidad después del alta. Los campos en la ficha son de solo lectura. | Confirmar la ausencia de pantalla. | A | `2.7-hueco-2-ficha-2.3-sin-edicion-matriz.png` | Trazado en Linear bajo el issue canónico **MOI-148** (marcando MOI-220 como duplicado). |
...
| H-23 | 4 | `/secretaria/certificaciones/nueva` | Catálogo de tipos de certificación sembrado con éxito en tenant nuevo (38 tipos activos, 0 con QES, conforme a política EAD Trust). Resuelto en **MOI-233**. Frontera restante en custodia final EAD Trust no construida en el alcance vigente. | Catálogo sembrado y operativo; frontera conocida en custodia EAD. | C | `4.7-certificacion-nueva.png`, Cloud DB `standalone_certification_kinds` = 38 | Resuelto catálogo en **MOI-233**. Frontera en custodia final trazada en **MOI-144**. |
...
| H-26 | 4 | `/secretaria/convocatorias/nueva` (Paso 8 - Emisión) | Convocatoria huérfana `64532414…` emitida con fecha de celebración en el pasado (2026-09-24 22:00 UTC) sin reunión materializada. **Resuelto**: rectificada formalmente en Cloud DB a estado `RECTIFICADA` vía `fn_transition_convocatoria_lifecycle`. Incorporados guards en `ConvocatoriasStepper.tsx` contra fechas pretéritas y duplicación concurrente en el mismo órgano/fecha (**MOI-232**). | Convocatoria rectificada canónicamente e idempotencia reforzada en cliente. | C | Cloud DB `convocatorias` (`RECTIFICADA`), `convocation_lifecycle_events`, test `convocatoria-idempotencia-guard.test.ts` | Resuelto en Cloud DB y trazado en **MOI-232**. |
