...
- Modify (identidad visible, hallazgos del review T1): `src/pages/secretaria/sociedad-nueva/StepIdentificacionLegal.tsx:8-11` (opción SLP en el dropdown de alta), `src/lib/secretaria/mesa-control-societaria.ts:763-770` (`LEGAL_FORM_LABELS_BY_JURISDICTION.ES` + clave SLP → "Sociedad Limitada Profesional"), `src/lib/secretaria/template-admin/labels.ts:104` (`TIPO_SOCIAL_LABEL` + SLP), `src/pages/secretaria/sociedad-nueva/StepReglas.tsx:67-72` (el pie `baseline.legalReference` no puede leerse como fuente del plazo: bajo la tarjeta "Convocatoria: 15 días" debe quedar claro que el plazo es LSC supletoria — p.ej. nota por-tarjeta o pie desdoblado; NUNCA la yuxtaposición "Ley 2/2007…" ↔ plazo)
...
- [ ] **Step 3b: Identidad visible (hallazgos review T1)** — opción `SLP` en el dropdown de `StepIdentificacionLegal.tsx` (label "Sociedad Limitada Profesional"); clave `SLP` en `LEGAL_FORM_LABELS_BY_JURISDICTION.ES` y en `TIPO_SOCIAL_LABEL` (que ninguna pantalla caiga al código crudo "SLP" donde las demás formas muestran nombre completo); en `StepReglas.tsx`, desambiguar el pie de `baseline.legalReference` para que la cita "Ley 2/2007…" no pueda leerse como fuente del plazo de convocatoria (regla del Comité Legal: el plazo cita solo LSC supletoria). Test: label del dropdown + labels maps con SLP.

- [ ] **Step 4: Ejecutar test + typecheck + `bun test`** — verde.

- [ ] **Step 5: (Controller) Backfill Cloud `entities.tipo_social='SLP'`** — G1 dejó las entidades SLP de Garrigues con `tipo_social` NULL y nota `TIPO_SOCIAL_SLP_PENDIENTE_G3` (`scripts/garrigues/entities-catalog.ts:45`); este es el punto del programa que lo salda. UPDATE dirigido: entidades del catálogo con `legalForm='SLP'` **y `jurisdiction='ES'`** (motor solo España — la SLP portuguesa y demás extranjeras NO se tocan) → `tipo_social='SLP'`. Verificar recuento contra el catálogo, ARGA intacta (0 filas suyas afectadas), y que `deriveTipoSocial` ya las reconoce (Step 3). Sin este backfill, la verificación viva de Task 9 no tiene entidad SLP real que abrir.
...
**Files:**
- Create: migración `<ts>_g3_junta_socios_pack_v110.sql` + actualizar `scripts/seed-garrigues-rule-packs.ts` — versión 1.1.0 de `GARR_JUNTA_SOCIOS` con referencias estatutarias firmes (arts. 27.3/27.4), desactivando v1.0.0.
...
- [ ] **Step 3: Implementar** — el pack v1.0.0 en Cloud ya trae `antelacionDias.SLP=15` y el canal individual (T3). El trabajo de T5 post-cotejo (2026-08-05) es subirlo a **v1.1.0 con las referencias estatutarias FIRMES**: antelación → `"arts. 27.4 Estatutos y 176 LSC (supletoria)"` con nota de la regla real de ampliación (**1 mes si el orden del día incluye modificación estructural** — la materia INTEGRACION la dispararía); canal → `"arts. 27.3 Estatutos (comunicación individualizada por escrito que asegure la recepción; también entrega en mano contra recibí)"`. Mecánica de versión: INSERT de `rule_pack_versions` v1.1.0 activa + desactivar v1.0.0 (espejo migración+seed, patrón T3); NUNCA mutar el payload de una versión ya aplicada. El "acuse" mantiene la semántica EAD etiquetada (externalidad no verificada — política 2026-07-21).
...
- Produces: cuando la Junta adopta una materia reservada, se emite un requisito `INFORME_PRECEPTIVO_ORGANO` (fase `PRE_CONVOCATORIA`, `blocking_policy='BLOCKING'`) que exige el informe del órgano informante (Consejo de Socios — informante único tras el cotejo con los Estatutos vigentes, ver más abajo) antes de emitir.

**Contenido EXACTO de `config.informe_preceptivo_de` (cotejado contra los ESTATUTOS VIGENTES el 2026-08-05 — firmeza FIRME; ver cotejo en el registro), sembrado SOLO en `garrigues-junta-socios`:**
...
Reglas post-cotejo: el informante de las 4 es el **Consejo de Socios** (el art. 39.5.b.ii da la elección/reelección del administrador único al CdS — el Comité de Nominaciones NO figura en Estatutos y queda fuera del gate); las claves `fuente`/`referencia`/`firmeza` son metadatos transparentes al helper (que solo casa materia + adoptante Junta); **ámbito exclusivo de la matriz** — las filiales NO reciben `informe_preceptivo_de` (y el art. 39.8 respalda el diseño: sin CdS, las referencias se tienen "por no puestas"); el perímetro estatutario REAL del informe es más amplio (39.5.b.i = todos los acuerdos 30.2 y 30.3) — el gate demo es un subconjunto correcto, ampliable post-demo añadiendo entradas. La copy del panel puede citar el art. 39.5.b con seguridad. Registro completo con el cotejo: `docs/legal/2026-08-04-decisiones-comite-legal-slp-garrigues.md`.

- [ ] **Step 1: Test del helper puro** — `informe-preceptivo-gate.ts` exporta `resolveInformePreceptivo(materia: string, organoAdoptante: string, config: BodyConfig): { requerido: boolean; organoInformante: string | null }`. Tests: `ADMISION_SOCIO_CUOTA` + Junta → `requerido:true, organoInformante:'garrigues-consejo-de-socios'`; `NOMBRAMIENTO_ADMINISTRADOR_UNICO` + Junta → `organoInformante:'garrigues-consejo-de-socios'` (post-cotejo: informante único, Consejo de Socios — el fixture del test cubre la lectura por entrada con un segundo slug ficticio de prueba, no un informante real distinto); materia no reservada → `requerido:false`; órgano no-Junta → `requerido:false`; entradas con claves extra `fuente`/`firmeza` → transparentes (no rompen el matching).
...
- [ ] **Step 2:** `scripts/seed-garrigues-templates.ts` service-role idempotente (capa1 inmutable + capa2 variables + capa3 editables; contenido legal = demo etiquetado; el texto literal de la carta de convocatoria real de §3.6 como capa 1). Vía TemplateImportWizard (ADMIN_TENANT de G0) o seed directo. **Regla de naming (hallazgo review T1):** los códigos/labels de las plantillas GARR que el selector del Paso 7 exhibe en crudo NO pueden decir "SL" para la Junta de una SLP (p.ej. `CONVOCATORIA_JUNTA_SOCIOS_SLP`, no `*_SL_*`); y si la capa 1 reutiliza el selector Handlebars `forma_social` SA/SL (migración `20260719210000`), verificar que la rama que una SLP alimenta (resuelve "SL" vía `ConvocatoriasStepper.tsx:1581`) renderiza texto correcto para la Junta de Socios — o usar estructura propia sin ese selector.
