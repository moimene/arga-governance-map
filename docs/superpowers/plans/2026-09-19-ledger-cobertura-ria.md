...
| F5 | ARRANCADA | **T14, T3 y el primer paso de T5 hechos el 20-09**, con autorización expresa del usuario para tocar la superficie compartida de GRC. Tres migraciones aplicadas y registradas, cada una con ensayo revertido previo y verificación que aborta: `20260920120000` (las **21** obligaciones de PBC/FT de Garrigues pasan de `risk` a `aml` — defecto vivo: la rama decía `OBL-GARR-PBC-%` y los códigos son `OBL-PBC-%`), `20260920130000` (módulo `ai` en los dos tenants + rama `OBL-RIA-%`; índice único `(tenant_id, code)`), `20260920140000` (`OBL-RIA-ORG-04`, art. 4, con órgano y política en los dos tenants). Gate nuevo **G-SYNC** (`grc-sync-modulos.test.ts`, 11 tests con logins reales), verificado por mutación. **Cambio visible declarado en ARGA:** 5 → 6 obligaciones y sección «Gobernanza de la IA» nueva. **Canal Cloud nuevo:** `psql` directo al pooler `aws-1-eu-central-1` con `DATABASE_PASSWORD` de `.env` — aplica desde el FICHERO, sin transcribir; el token del keychain para la Management API está caducado («JWT could not be decoded»). **Pendiente de T5:** el control del art. 4 no se crea todavía (`controls.status` solo admite Efectivo/Parcial/Inefectivo y no hay valor para «sin probar»: crearlo obligaría a afirmar una efectividad no medida). Llega con T6, el registro de formación |
| F3, F4, F6-F11 | PENDIENTE | |
