...
- **Deuda anotada (no G0, resuelta 2026-09-24 en MOI-127):** `src/lib/demo-operable/runner.ts` desacoplado del hardcode `ARGA_TENANT_ID` (DemoRunOptions propaga tenantId/entityId del contexto de sesión, sin atribución automática a ARGA, y `DemoScenarioResult.tsx` cuenta con guarda fail-closed que restringe el acceso a ARGA); placeholders `@arga-seguros.com` en varios steppers/GRC/AI (contenido cosmético pre-existente, no theming).
