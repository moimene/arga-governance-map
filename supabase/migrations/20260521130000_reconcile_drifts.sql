-- RECONCILIACIÓN DRIFT DR2: Declaración formal del WORM trigger en el repositorio
CREATE OR REPLACE FUNCTION public.worm_guard()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Vincular trigger WORM a evidence_bundles
DROP TRIGGER IF EXISTS evidence_bundles_worm_guard ON public.evidence_bundles;
...
-- RECONCILIACIÓN DRIFT DR1: Reinstaurar fn_save_meeting_resolutions con extensions.digest
CREATE OR REPLACE FUNCTION public.fn_save_meeting_resolutions(
  p_tenant_id uuid,
  p_meeting_id uuid,
  p_rows jsonb
) RETURNS jsonb
...
BEGIN
  ...
  LOOP
    ...
    -- Hardening (Codex review #4): si el agreement_id procede del payload del caller,
    -- debe pertenecer a ESTA reunión. Evita que un SECRETARIO del tenant relink o
    -- sobrescriba un acuerdo ajeno enviando su id en el payload de guardado.
    IF v_agreement_id IS NOT NULL THEN
      PERFORM 1 FROM agreements
       WHERE id = v_agreement_id
         AND tenant_id = p_tenant_id
         AND parent_meeting_id = p_meeting_id;
      IF NOT FOUND THEN
        RAISE EXCEPTION 'agreement % no pertenece a la reunion % (tenant %)',
          v_agreement_id, p_meeting_id, p_tenant_id;
      END IF;
    END IF;
    ...
    IF v_action = 'UPSERT' THEN
      ...
      IF v_agreement_id IS NULL THEN
        INSERT INTO agreements (
          ...
          p_tenant_id,
          -- Hardening (Codex review #4): entity_id/body_id derivados de la reunión
          -- bloqueada (v_meeting), no del payload del caller.
          v_meeting.entity_id,
          v_meeting.body_id,
      ...
      ELSE
        UPDATE agreements
           SET entity_id = v_meeting.entity_id,
  ...
  LOOP
    ...
    IF v_resolution_agreement_id IS NOT NULL AND v_snapshot IS NOT NULL AND jsonb_typeof(v_snapshot) = 'object' THEN
      ...
      IF v_snapshot #>> '{rule_trace,source}' = 'V2_CLOUD'
        ...
        -- USO ESTÁNDAR DE extensions.digest PARA ELUDIR SEARCH PATH ISSUES
        v_eval_hash := encode(extensions.digest(v_eval_without_hash::text, 'sha256'), 'hex');
...
GRANT EXECUTE ON FUNCTION public.fn_save_meeting_resolutions(uuid, uuid, jsonb) TO authenticated, service_role;
