-- Secretaria demo-readiness closeout: NO_SESSION source-of-truth hardening.
--
-- Fixes two integrity gaps:
-- 1. A Secretario/Admin may record a documented response for a target person,
--    but the WORM row is still keyed by that target person_id.
-- 2. Materialization no longer trusts client-provided APROBADO. Approval is
--    recomputed from no_session_respuestas before an agreement is created.
